<?php 
  ...
  if(isset($_GET["search"])){
    ...
    $sql = "SELECT p.id, p.name, CASE WHEN p.status = 1 THEN 'In Stock' WHEN p.status = 0 THEN 'Out Of Stock' END AS Status, p.price, p.sale_price, ct.name as Category, p.image FROM products p INNER JOIN category ct ON p.category_id = ct.id WHERE p.name LIKE '%$search%'";
